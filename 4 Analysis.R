...
stopw = c(stopwords(language = "ru"), 'это')
...
a=ggplot() +
  geom_histogram(data = beforesent, aes(x = score, y=..density..), fill="#008080", col="#483D8B", alpha = 0.5)+ggtitle('?? 24.02')+xlab('????????????? ??????? (?? ?????????? ? ?????????????)')+ylab('????????? ?????????????')+ylim(0,2)
b=ggplot() +
  geom_histogram(data = aftersent, aes(x = score, y=..density..), fill="#008080", col="#483D8B", alpha = 0.5)+ggtitle('????? 24.02')+xlab('????????????? ??????? (?? ?????????? ? ?????????????)')+ylab('????????? ?????????????')
SCATS <- plot_grid(a, b)
SCATS
